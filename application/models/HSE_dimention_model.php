...
class HSE_dimention_model extends CI_Model
{
	...
	{
		return $dba->get($schema . '.HSE_DATA_DIMENTION')->result();
	}
	...
	public function getByFactory($schema, $dba)
	{
		...
		return $dba->get_where($schema . '.HSE_DATA_DIMENTION', ['FACT_NO' => $factory])->result();
	}

	// function getById data on the table
	public function getById($schema, $dba, $fact_no)
	{
		return $dba->get_where($schema . '.HSE_DATA_DIMENTION', ['FACT_NO' => $fact_no])->result();
	}
}
