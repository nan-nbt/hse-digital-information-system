...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed text-sm">
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-12">
							<div class="card">
								<div class="card-body">
									<table id="table-aspect" class="table table-bordered table-striped table-head-fixed nowrap text-nowrap" style="width: 100%;">
										<thead>
											<tr>
												<th>NO</th>
												<th>DIMENTION NO</th>
	...
	<script>
		...
		function loadDataAspect() {
			$.ajax({
				url: "<?php echo base_url('dashboard/dataAspect'); ?>",
				async: true,
				dataType: 'JSON',
				success: function(data) {
					...
					if (data.length > 0) {
						...
						$('#table-aspect').DataTable().destroy();
						$('#list-aspect').html(htmlDataAspect);

						// set value object of datatables
						g_table = $('#table-aspect').DataTable({
							"pagging": true,
							"lengthChange": true,
							"searching": true,
							"ordering": true,
						...
						g_table.buttons().container().appendTo('#table-aspect_wrapper .col-md-6:eq(0)');

						// function on row data click get data row
						$('#table-aspect tbody').off('click').on('click', 'tr', function() {
						...
						// set value object of datatables
						g_tablem = $('#table-aspect').DataTable({
							"paging": true,
							"lengthChange": true,
							"searching": false,
							"ordering": true,
						...
						g_tablem.buttons().container().appendTo('#table-aspect_wrapper .col-md-6:eq(0)');
		...
		// set interval
		setInterval(loadDataAspect, 120000);
	</script>
