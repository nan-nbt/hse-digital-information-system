...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed text-sm">
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-12">
							<div class="card">
								<div class="card-body">
									<table id="table-build" class="table table-bordered table-striped table-head-fixed nowrap text-nowrap" style="width: 100%;">
										<thead>
											<tr>
												<th>NO</th>
												<th>AREA NO</th>
	...
	<script>
		...
		function loadDataBuild() {
			$.ajax({
				url: "<?php echo base_url('dashboard/dataBuild'); ?>",
				async: true,
				dataType: 'JSON',
				success: function(data) {
					...
					if (data.length > 0) {
						...
						$('#table-build').DataTable().destroy();
						$('#list-build').html(htmlDataBuild);

						// set value object of datatables
						g_table = $('#table-build').DataTable({
							"pagging": true,
							"lengthChange": true,
							"searching": true,
							"ordering": true,
						...
						g_table.buttons().container().appendTo('#table-build_wrapper .col-md-6:eq(0)');

						// function on row data click get data row
						$('#table-build tbody').off('click').on('click', 'tr', function() {
						...
						// set value object of datatables
						g_tablem = $('#table-build').DataTable({
							"paging": true,
							"lengthChange": true,
							"searching": false,
							"ordering": true,
						...
						g_tablem.buttons().container().appendTo('#table-build_wrapper .col-md-6:eq(0)');
		...
		// set interval
		setInterval(loadDataBuild, 120000);
	</script>
